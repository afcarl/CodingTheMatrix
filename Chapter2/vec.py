...
def equal(u,v):
    ...
    # if (sum(v.f.values()) == sum(u.f.values()) == 0):
    #     return True 

    if u_new.keys() == v_new.keys():

        if u_new.values() == v_new.values():
            return True
        else:
            return False
    else:
...
def add(u,v):
    ...
    u_new = dict([[k, u.f[k]] for k in u.f.keys() if u.f[k] != 0])
    v_new = dict([[k, v.f[k]] for k in v.f.keys() if v.f[k]!= 0])
    v_not_u = set(v_new.keys()) - set(u_new.keys())
    u_not_v = set(u_new.keys()) - set(v_new.keys())
    u_v = set(u_new.keys()) & set(v_new.keys())

    v_not_u_list = [[x,v_new[x]] for x in v_not_u]
    u_not_v_list = [[x,u_new[x]] for x in u_not_v]
    u_v_list = [[x, u_new[x]+ v_new[x]] for x in u_v]
    return Vec( u.D,dict(v_not_u_list + u_not_v_list + u_v_list))
...
def dot(u,v):
    ...
    """
    assert u.D == v.D
    if len(u.f.keys()) != len(v.f.keys()):
        values_not_present_in_both = u.f.keys() ^ v.f.keys()
...
def neg(v):
    ...
    new_f = dict([(x,-y) for (x,y) in v.f.items()])
    return Vec(set(new_f.keys()), new_f)
    #v.f.values() = (-1*i for i in v.f.values())
...
if __name__ == '__main__':
    v = Vec({1,2,3},{1:2,2:3,3:4})
    #print v.D
    #print v.f
    #print getitem(v,2)
    #print setitem(v,1,10)
    #print -v
    #print scalar_mul(v, 0)
    #print v*v
    b = Vec({'a','e','i','o','u'}, {'o':4,'u':7})
    d = Vec({'x','y','z'}, {'x':2,'y':1})
    e = Vec({'x','y','z'}, {'z':4,'y':-1})
