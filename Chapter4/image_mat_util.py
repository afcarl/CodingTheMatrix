...
def mat2display(pts, colors, row_labels = ('x', 'y', 'u'),
    ...
    openinbrowser('file://%s' % hpath)
    print("Hit Enter once the image is displayed.... ")
    input()


path = "/Users/khokher/Documents/CodingTheMatrix/Chapter4/icon.png"
image_matrix = file2mat(path)
# I = matutil.identity({'x','y','u'},1)
# image_result =  matrix_matrix_mul(I,image_matrix[0])
# mat2display(image_result,image_matrix[1])

A = matutil.rowdict2mat({'x':Vec({'x','y','u'},{'x':1, 'y':0,'u':0}),'y':Vec({'x','y','u'},{'x':0, 'y':1,'u':0}),'u':Vec({'x','y','u'},{'x':0, 'y':0,'u':1})})
image_result =  matrix_matrix_mul(A,image_matrix[0])
mat2display(image_result,image_matrix[1])

#mat2display(image_matrix[0],image_matrix[1])
