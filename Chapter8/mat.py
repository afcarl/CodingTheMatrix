...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    dict_A  = dict(filter(lambda x: x[1]!=0, A.f.items()))
    dict_B  = dict(filter(lambda x: x[1]!=0, B.f.items()))
    if dict_A==dict_B:
        return True
...
def scalar_mul(M, x):
    ...
    True
    """
    M.f.update((a,b*x) for a,b in M.items())
    return M
...
def vector_matrix_mul(v, M):
    ...
    for i in range(len(v_i)):
        factor = v_i[i][1]
        row_var = v_i[i][0]
        row = filter(lambda x: x[0][0]==row_var, m_i)
        row = map(lambda x: (x[0],x[1]*factor), row)
        row = map(lambda x: (x[0][1],x[1]), row)
        list_.append(row)
    list_ = [item for sublist in list_ for item in sublist]
    for key, val in list_:
        testDict[key] += val

    result.f = dict(testDict)
    return result
...
def matrix_matrix_mul(A, B):
    ...
    v = Vec(B.D[0],{})
    for i in B.D[1]:
        extract_from_dict = filter(lambda x: x[0][1]==i ,B.f.items())
        v.f = dict(map(lambda x: (x[0][0],x[1]), extract_from_dict))
        # print v
        # print A
        values = matrix_vector_mul(A.copy(),v).f
        values_new = dict(map(lambda x: ((x[0],i),x[1]), values.items()))
        C.f.update(values_new)
